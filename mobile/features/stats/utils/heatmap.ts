...
export function getRecentHeatmapDays(days: HeatmapDay[], count = STATS_HEATMAP_RECENT_DAYS): HeatmapDay[] {
  if (days.length <= count) return days;
  return days.slice(-count);
}

export function hasRecentHeatmapActivity(days: HeatmapDay[], count = STATS_HEATMAP_RECENT_DAYS): boolean {
