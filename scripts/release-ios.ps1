...
function Invoke-NpxEas([string]$Arguments) {
    & cmd.exe /d /s /c "npx eas $Arguments"
    return $LASTEXITCODE
}
...
function Invoke-Preflight {
    ...
    try {
        $whoami = & cmd.exe /d /s /c "npx eas whoami 2>&1"
...
function Start-EasBuild {
    ...
    Push-Location $MobileDir
    try {
        $exitCode = Invoke-NpxEas "build --platform ios --profile production --non-interactive"
        if ($exitCode -ne 0) {
...
function Start-EasSubmit {
    ...
    try {
        if ($SubmitLatest) {
            $exitCode = Invoke-NpxEas "submit --platform ios --profile production --latest --non-interactive"
        } elseif ($Id) {
            $exitCode = Invoke-NpxEas "submit --platform ios --profile production --id $Id --non-interactive"
        } else {
            throw 'Pass -BuildId BUILD_ID or -SubmitLatest.'
        }
        if ($exitCode -ne 0) {
